...
| Feature | Description |
|---------|-------------|
| **Custom GraphRAG** | Built from scratch with Neo4j — not a wrapper around existing frameworks |
| **Patient Personalization** | Risk scores adjusted for age, kidney/liver function, pregnancy, polypharmacy |
| **Text-to-Cypher Engine** | Natural language → graph queries with self-healing retry mechanism |
...
├── data/
│   ├── raw/
│   │   └── db_drug_interactions.csv   # Source drug interaction data
│   ├── synthetic/
│   │   ├── patients.json              # 752 synthetic patients
│   │   └── test_cases.json            # 48 evaluation test cases
│   └── evaluation/
...
├── docs/
│   └── screenshot.png                 # App screenshot for landing page
├── .env                               # Environment variables
├── .gitignore
├── docker-compose.yml                 # Neo4j container config
├── requirements.txt                   # Python dependencies
├── MedGraphRAG_Documentation          # Full technical documentation
...
**Output Files:**
- `data/evaluation/metrics.json` — Machine-readable metrics
- `data/evaluation/evaluation_report.txt` — Human-readable report
...
| Score | Level | Action |
|-------|-------|--------|
| ≥ 70 | Critical | Immediate medication review |
| ≥ 50 | High | Consult pharmacist |
...
- **[Technical Documentation (PDF)](./DOCUMENTATION.pdf)** — Full system architecture, implementation details, evaluation methodology
- **[Landing Page](https://pranavkharat.github.io/medgraphrag/)** — Project showcase with interactive elements
...
- [DrugBank](https://go.drugbank.com/) — Drug interaction data source
